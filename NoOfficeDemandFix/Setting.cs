    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.DiagnosticsSamplesPerDay)), "Controls how many diagnostic sample slots exist per displayed in-game day. When CaptureStableEvidence or VerboseLogging keeps diagnostics emitting, each slot is recorded once and sample_count follows the configured cadence. This only matters when diagnostics are enabled. Default is 2, and time-scaling mods still respect the displayed day when choosing sample slots." },
