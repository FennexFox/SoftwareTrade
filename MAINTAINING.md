...
- Treat `3 days` as the default minimum software-gate window for release decisions. For zero-weight virtual-resource checks where the main question is current-default stability rather than long-horizon persistence, a tightly bounded `2-day` same-lineage comparison may still be acceptable if the release summary records that shorter gate explicitly. Prefer `5 days` only when buyer-lifecycle or persistence interpretation remains materially ambiguous after the shorter gate window.
