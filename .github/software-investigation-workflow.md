...
1. copy `environment(settings=..., patch_state=...)`
2. copy `diagnostic_counters(...)`
3. use `softwareEvidenceDiagnostics observation_window(...)` for the observation window
4. store relevant `softwareEvidenceDiagnostics detail(...)` lines in artifacts or notes when property-level state matters
5. keep interpretation separate from capture as much as practical
...
4. Record a minimal comparison result for each checkpoint used.
   Each comparison result should record the checkpoint, baseline reference, comparison reference, invariant status, observed deltas, and notes.
