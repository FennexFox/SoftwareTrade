...
- verbose `softwareEvidenceDiagnostics detail(...)` with `detail_type=softwareTradeLifecycle` when lifecycle transitions or seller snapshots are captured, plus `detail_type=softwareVirtualResolutionProbe` when checking the discussion-`#63` zero-weight virtual fast-path hypothesis, and `detail_type=softwareBuyerTimingProbe` when checking whether a selected need stayed below threshold across enough windows to justify a corrective buyer pass; treat all three as supplemental artifacts rather than as a replacement for scheduled observation-window anchors. Keep high-volume fix-specific probes such as `virtualOfficeBuyerFixProbe` and lightweight seller-eligibility probes such as `outsideConnectionVirtualSellerProbe summary(...)` as supplemental artifact material unless the active investigation is explicitly about probe volume, override sizing, or seller eligibility itself; the verbose `outsideConnectionVirtualSellerProbe sample(...)` lines are seller-snapshot artifacts rather than baseline evidence anchors
...
- supplemental artifacts: `outsideConnectionVirtualSellerProbe summary(...)` for office-import request volume and, when verbose capture was enabled for a seller-eligibility check, `outsideConnectionVirtualSellerProbe sample(...)` for seller snapshot counts such as missing stored-resource pairs
