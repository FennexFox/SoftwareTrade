...
| `EnableOfficeDemandDirectPatch` | `true` | Restores the pre-1.5.6f1 office demand baseline with the shipped direct Harmony patch. Turn it on before launch when you need like-for-like comparisons against earlier evidence, and leave it on for the current release unless you are intentionally comparing against the newer vanilla `3x` baseline. |
