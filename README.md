...
| `DiagnosticsSamplesPerDay` | `2` | Sets how many scheduled diagnostic sample slots exist per displayed in-game day. Emitted `observation_window(...)` lines report the slot they actually sampled, `sample_count` counts emitted observations in the current run, and `skipped_sample_slots` reports scheduled gaps that were not backfilled. |
